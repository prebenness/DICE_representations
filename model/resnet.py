...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, num_channels=3):
        ...
        self.in_planes = 64
        self.network_type = cfg.DATASET.NAME
        self.final_pool_size = 4 if not self.network_type == 'PCAM' else 7

        self.conv1 = nn.Conv2d(num_channels, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        #first_stride = 1 if not cfg.DATASET.NAME == 'PCAM' else 2
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out = F.avg_pool2d(out, self.final_pool_size)
